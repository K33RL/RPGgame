...
public class Map {
    ...
    public Map() {
        ...
        for (int i = 0; i < 15; i++) {
            int cellX = MathUtils.random(0, CELLS_Y - 1);
            int cellY = MathUtils.random(0, CELLS_Y - 1);
            dataLayer[cellX][cellY] = 1;
            typeLayer[cellX][cellY] =(byte) MathUtils.random(0,3);
        }
        textureGrass = new Texture(Gdx.files.internal("Grass.png"));
        textureWalls = new Texture(Gdx.files.internal("walls.png"));
        regions = new TextureRegion(textureWalls).split(100, 100)[0];
    }

    public boolean isCellOPassable(Vector2 position) {
